...
class Parser:
  ...
  def __init__(self,
    ...
    if buff:
      ...
      if soup.find("pre"):
        ...
        self.data_lines = re.split("(?:<[Bb][Rr]\s*?/?>)", self.raw_data)
        self.data_lines = [re.sub("[\r\n]+", " ", line) for
    ...
    self.results = [Result(line, date) for line in self.data_lines]
    self.set_results()
  ...
  def get_class_words(self):
    ...
    class_words = {key:value for key, value in class_words.items() if
  ...
  def get_class(self, line):
    ...
    """
    if self.class_index:
      for class_word, class_year in self.class_words.items():
